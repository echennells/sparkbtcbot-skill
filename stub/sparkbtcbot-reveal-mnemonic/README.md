...
`sparkbtcbot-reveal-mnemonic` was a command in `sparkbtcbot-skill` 0.4.x-0.5.x. Version 0.6.0 replaced the five
...
`npx <cmd>` resolves whatever you type as an npm package name. If `sparkbtcbot-reveal-mnemonic` were
unregistered, a bare `npx sparkbtcbot-reveal-mnemonic` — typed from memory, an old tutorial, or by an AI agent
